...
const CytoscapeRendering = ({ graph, layout }) => {
  ...
  useEffect(() => {
    // re-map cyStyles
    const updatedStyles = updateProductEdgeThickness(
      appSettings.productEdgeThickness
    );

    // update cytoscape styles based on app settings
    const style = mapStylesToCytoscape(updatedStyles, appSettings);
  ...
  }, [graph, layout, showReagents, duplicateReagents]);
